...
class AccettareRichiestaAmiciziaTest extends GenericTest {
	...
	private function callController($userid, $from_user_id) {
		...
		ob_start();
		include "../../controllers/amicizia/Accetta richiesta amicizia.php";
		$response = ob_get_contents();
		ob_end_clean();
		return $response;
	}

	public function test_TC_3_2_1() {
		$response = $this->callController(null, self::$user2_id);
		$this->assertEquals($response, "Il client non ti ha bloccato?");
	}

	public function test_TC_3_2_2() {
		$response = $this->callController(self::$user1_id, null);
		$this->assertEquals($response, "Il client non ti ha bloccato?");
	}

	public function test_TC_3_2_3() {
		$response = $this->callController(self::$user1_id, self::$user2_id);
		$this->assertEquals($response, "ok");
		$this->assertTrue(self::$amicizia_dao->existsFriendshipBetween(self::$user1_id, self::$user2_id));
		$this->assertTrue(self::$amicizia_dao->removeFriendshipBetween(self::$user1_id, self::$user2_id));
	}
